...
* https://kubernetes.io/docs/user-guide/configmap/[Kubernetes `ConfigMap`]
    ** allows
        *** externalize parameters -- to -- your application -- via --
            **** key-value pairs or
            **** embedded `application.properties` or `application.yaml`
* link:https://github.com/spring-cloud/spring-cloud-kubernetes/tree/main/spring-cloud-kubernetes-fabric8-config[Spring Cloud Kubernetes Config]
    ** makes Kubernetes `ConfigMap` instances / AVAILABLE | application startup
    ** 👀if changes are detected | observed `ConfigMap` instances -> trigger HOT reloading of beans or Spring context 👀

* ALL NEXT features valid for ConfigMap & Secret

* default behavior
    ** create a `Fabric8ConfigMapPropertySource` (or `KubernetesClientConfigMapPropertySource`) -- based on a -- Kubernetes `ConfigMap` / `metadata.name` contains
        *** `spring.cloud.kubernetes.config.name` 's value
        *** `spring.application.name` 's value
        *** String literal `"application"`
* 👀if you want to use MULTIPLE `ConfigMap` instances -> use `spring.cloud.kubernetes.config.sources`👀
    ** _Example:_
+
[source,yaml]
----
      ...
      config:
        ...
        sources:
         ...
         - namespace: n3
           name: c3
----
    ** if `spring.cloud.kubernetes.config.namespace` NOT set -> `ConfigMap` `c1` -- would be looked up in the -- namespace / application runs
        *** see xref:property-source-config/namespace-resolution.adoc[Namespace resolution]
